"""
SPEED Component for exporting OSMs - Modifed version of Honeybee ExportToOSM
_
...
ghenv.Component.Name = "SPEED Export to OpenStudio"
...
def main(HBZones, HBContext, north,epwWeatherFile = None, analysisPeriod = [], simParameters = [], simulationOutputs = []):
    ...
    # Make path of SPEED directory
    
    ## os.getenv("USERNAME") is not portable between environments e.g will only work on Windows
    
    SPEEDDirectory = os.path.join('C:/Users',os.getenv("USERNAME"),'AppData/Roaming/Grasshopper/SPEED')
    
    # Directory should exist as installer will make it!
    if not os.path.exists(SPEEDDirectory):
        
        # mk the dir
        try:
            os.mkdir(SPEEDDirectory)
        except:
            print "Could not make the directory "+SPEEDDirectory+" ! failed to write SPEED OSM files out there"
            return
            
    # Get Name from SPEED sliders
    ...
    fname = os.path.join(SPEEDDirectory,thisOSMname+ ".osm")
    #save the model
...
# Make a set of connected SPEED sliders 

if _HBZones and _HBZones[0]!=None:
    
    main(HBZones, HBContext, north)